...
create_snphub_object <- function(snp_data, snp_summary, snp_alleles = NULL, output_dir = NULL) {
	obj <- list(
		snp_data = snp_data,
...
#' @export
snp_summary <- function(x, ...) {
	cat("\n--- Data Summary ---\n")
	cat("Number of markers (SNPs):", nrow(x$snp_data), "\n")
	...
	cat("Heterozygous SNPs:", x$snp_summary[["Hets"]], "\n")
